...
function Partner() {
  ...
  return (
    <Grid sx={{ width: "85vu", margin: "auto" }} container spacing={2}>
      ...
      <Grid sx={{cursor:"pointer"}} onClick={()=>handleNavigate("https://gempad.app/presales")} item xs={6} md={3}>
        
        <img src={gemped} alt="gemped logo" />
      </Grid>
      <Grid sx={{cursor:"pointer"}} onClick={()=>handleNavigate("https://hedgepay.org/")} item xs={12} md={3}>
        <img src={hedgepay} alt="hedpay logo" />
      </Grid>
      <Grid sx={{cursor:"pointer",marginTop:"0.7em"}} onClick={()=>handleNavigate("https://presale.world/")} item md={3}>
        <img src={presale} width="70%" alt="presale logo" />
      </Grid>
      <Grid sx={{cursor:"pointer"}} onClick={()=>handleNavigate("https://www.lbank.info/")} item xs={12} md={3}>
        <img src={lbank} alt="lbank logo" />
      </Grid>
      <Grid sx={{cursor:"pointer"}} onClick={()=>handleNavigate("https://luxpad.io/")} item xs={12} md={3}>
        <img src={luxpad} alt="luxpad logo" />
      </Grid>
      <Grid sx={{cursor:"pointer"}} onClick={()=>handleNavigate("https://newtokenmarket.com/")} item xs={12} md={3}>
        <img src={newToken} alt="new token logo" />
      </Grid>
      <Grid sx={{cursor:"pointer"}} onClick={()=>handleNavigate("https://pandasale.finance/")} item xs={12} md={3}>
        <img src={pandasale} alt="panda sale logo" />
      </Grid>
   

      <Grid sx={{cursor:"pointer"}} onClick={()=>handleNavigate("https://bitcourier.co.uk/news/expelee-interview")} item md={3}>
      ...
      <Grid item md={3} xs={12} sx={{cursor:"pointer"}} onClick={()=>handleNavigate("https://www.probit.com/en-us/")}>
      <img src={probit} alt="probit logo" />
      </Grid>
      <Grid sx={{cursor:"pointer"}} onClick={()=>handleNavigate("https://hami.network/")} item xs={12} md={3}>
        <img src={hamiNetwork} alt="hami network logo" />
      </Grid>
     
    

      <Grid sx={{cursor:"pointer"}} onClick={()=>handleNavigate("https://t.me/phoenixholdings")} item xs={12} md={3}>
